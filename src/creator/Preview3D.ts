...
export class Preview3D {
  ...
  private onMouseDown = (e: MouseEvent): void => {
    ...
    if (hits.length > 0) {
      ...
      while (obj.parent && !obj.userData.layerId) obj = obj.parent
      const id = obj.userData.layerId as string
      if (!id) return
  ...
  private typeColor(type: string): number {
    const c: Record<string, number> = { image: 0x4488ff, video: 0xe63329, gif: 0xff9900, glb: 0xaa44ff, gltf: 0xaa44ff, model: 0xaa44ff, svg: 0x00cc88, texto: 0xffffff }
  ...
  private animate = (): void => {
    ...
    this.layers.forEach(entry => {
      if (!entry.glbLoaded || !entry.mesh.parent) return
      const l = entry.layer
