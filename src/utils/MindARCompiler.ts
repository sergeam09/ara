...
async function getCompiler(): Promise<any> {
  ...
  const mod = await dynamicImport('https://cdn.jsdelivr.net/npm/mind-ar@1.2.5/dist/mindar-image.prod.js') as any

  const Compiler = mod.Compiler || mod.default?.Compiler || (window as any).MINDAR?.IMAGE?.Compiler
...
export async function compileTriggerImage(
  imageFile: File,
