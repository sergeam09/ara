import { compileTriggerImage } from '@/utils/MindARCompiler';
...
export class Publisher {
    ...
    async publishProject(project, projectName, onProgress) {
        const events = [];
        const emit = (event) => {
            events.push(event);
            if (onProgress)
        ...
        try {
            ...
            if (modo === 'image' && project.trigger) {
                ...
                const mindData = await compileTriggerImage(project.trigger, (progress) => {
